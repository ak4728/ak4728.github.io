(function() {
  ...
  ASSET_URL = 'https://rabona.io/img/flag/';
  ...
  $.getJSON({
    ...
    success: function(data){
      ...
      lineup2 = JSON.parse(data.lineup_team_2);
      formation1 = data.formation_team_1;
      formation2 = data.formation_team_2;
    }
  });
  

  append_json(lineup);
  function append_json(lineup){
    lineup.forEach(function(object) {
      ///console.log(object.overall_strength);
      if(object.number<12){
        $.getJSON({type: "GET",url: apiserver +'player?uid='+object.uid,async: false,success: function(data){player = data;}});
        data.players.home[object.number-1].name = object.name;
        data.players.home[object.number-1].x = formation[formation1][object.number-1]['x'];
        data.players.home[object.number-1].y = formation[formation1][object.number-1]['y'];
        data.players.home[object.number-1].asset = flagLinks[object.country];
        data.players.home[object.number-1].origin = object.country;
        data.players.home[object.number-1].age = Math.ceil(player.age);
        data.players.home[object.number-1].os = Math.ceil(player.overall_strength);
      }      
    });
  }

  append_json2(lineup2);
  function append_json2(lineup2){
    lineup2.forEach(function(object) {
      ///console.log(object.overall_strength);
      if(object.number<12){
        $.getJSON({type: "GET",url: apiserver +'player?uid='+object.uid,async: false,success: function(data){player = data;}});
        data.players.away[object.number-1].name = object.name;
        data.players.away[object.number-1].x = formation[formation2][object.number-1]['x'];
        data.players.away[object.number-1].y = formation[formation2][object.number-1]['y'];
        data.players.away[object.number-1].asset = flagLinks[object.country];
        data.players.away[object.number-1].origin = object.country;
        data.players.away[object.number-1].age = Math.ceil(player.age);
        data.players.away[object.number-1].os = Math.ceil(player.overall_strength);
      }      
    });
  }
  ...
  dom = {
    ...
    populateCard: function($el, data) {
      return $el.append('<h3>' + data.name + '</h3>' + '<ul class="player__card__list"><li><span>AGE</span><br/>' + data.age + '</li><li><span>Origin</span><br/>' + data.origin + '</li></ul>' + '<ul class="player__card__list player__card__list--last"><li><span>OS</span><br/>' + data.os  + '</li></ul>');
    },
    /// + '</li><li><span>Goals</span><br/>' + data.goals /// ' yr</li><li><span>Height</span><br/>' + data.height + 
